...
Object.type = function(obj){
	...
	if (toString.call(obj) === '[object MediaList]') return 'object';
	if (toString.call(obj) === '[object CSSRuleList]') return 'object';
	if (toString.call(obj) === '[object CSSStyleDeclaration]') return 'style';
	if (toString.call(obj) === '[object Array]') return 'array';
	if (toString.call(obj) === '[object Date]') return 'date';
	if (obj.nodeName){
		switch (obj.nodeType){
...
String.camelCase = (function(){
	// From MooTools (modified)
	function upCaseChar(match){
		return match.charAt(1).toUpperCase();
	}
	return function(string){
		return ('' + string).replace(/-\D/g, upCaseChar);
	}
}());

var hasOwnProperty = {}.hasOwnProperty;
var ppd = {}
function pp(object, depth){
	if (!depth) depth = 0
	// if (depth > 4) return "BLARGH"
	...
	// 	var tmp = {}
	// 	for (var property in object){
	// 		if (!hasOwnProperty.call(object, property)) continue;
	// 		tmp[property] = String(object[property])
	// 	}
	// 	object = tmp
		// break;
	case 'style':
	...
	case 'object':
		...
		for (var key in object) {
			...
			var value = object[key];
			h += '<tr><th> '+key+' </th><td> '+pp(value, depth + 1)+' </td></tr>';
			keys[key] = true;
			if (key == (+key)){
				if (object[value] && !keys[value]){
					h += '<tr><th> '+value+' </th><td> '+pp(object[value], depth + 1)+' </td></tr>';
					keys[value] = true
				}
				var valCamelCase = String.camelCase(value)
				if (valCamelCase != value && object[valCamelCase] && !keys[valCamelCase]){
					h += '<tr><th> '+valCamelCase+' </th><td> '+pp(object[valCamelCase], depth + 1)+' </td></tr>';
					keys[valCamelCase] = true
				}
			}
		}
		h += '</table>';
	...
	case 'regexp':  return '<span class="regexp">'  + String.escapeHTML(object)+'</span>'; break;
	case 'function':return '<code class="function">'+ String.escapeHTML(object)+'</code>'; break;
